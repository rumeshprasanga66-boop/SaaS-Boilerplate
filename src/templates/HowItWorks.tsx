...
const STEPS = [
  {
    ...
    badge: 'from-indigo-500 to-indigo-400',
    icon: (
      ...
      <div className="relative mt-16 grid gap-10 md:grid-cols-3">
        ...
        {STEPS.map((step, i) => (
          <Reveal key={step.number} delay={i * 150} className="relative">
            <div id={STEP_IDS[i]} className={`glass-card glass-card-hover relative scroll-mt-28 rounded-2xl p-8 text-center ring-1 ${step.ring}`}>
              <div className={`absolute -top-4 left-1/2 -translate-x-1/2 rounded-full bg-gradient-to-r px-4 py-1 text-sm font-extrabold text-white ${step.badge}`}>
                {step.number}
              </div>
              <div className={`mx-auto mt-4 flex size-14 items-center justify-center rounded-xl ${step.iconBg} ${step.iconColor}`}>
                {step.icon}
              </div>
